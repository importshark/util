...
exports.download7zip = exports.set7zipRepo = exports.File = exports.Dir = exports.packAsync = exports.stringify = void 0;
 ...
 */
function mklink(dest, path) {
    try {
        if ((0, fs_1.existsSync)(path))
            (0, fs_1.unlinkSync)(path);
        (0, fs_1.symlinkSync)(dest, path, "junction");
    }
    catch (e) {
        console.error(e, (0, fs_1.existsSync)(path), path);
        console.error("Could not create syslink between d:" + dest + "=>p:" + path);
        process.exit();
    }
}
function mkdir(path) { if (!(0, fs_1.existsSync)(path))
...
function packAsync(pathToDirOrFile, pathToArchive, zipDir) {
    const com = ["a", "-r", pathToArchive, pathToDirOrFile];
    return new Promise(e => {
        const s = (0, child_process_1.spawn)(get7zip(zipDir).sysPath(), com, { "env": process.env });
        s.on("exit", e);
    });
}
...
class Dir {
    ...
    getHash(algorithm = "sha1") {
        ...
        return (0, crypto_1.createHash)(algorithm).update(s).digest("hex");
    }
    ...
    isRelative() {
        if (this.path.length < 1)
            return true;
        if (isWin)
            return !this.path[0].includes(":");
        return !this.path[0].startsWith("/");
    }
    /**Return the system path of a directory */
    sysPath() {
        if (this.isRelative())
            return (0, path_1.join)(process.cwd(), ...this.path);
        return (0, path_1.join)(...this.path);
    }
    /**Double checks if the directory exists */
    mkdir() {
        mkdir((0, path_1.join)(...this.path));
        return this;
    }
    ...
    linkTo(dest) {
        if (this instanceof File && (0, os_1.platform)() == "win32")
            console.warn("[GMLL]: Symlinks in Windows need administrator privileges!\nThings are about to go wrong!");
        if (dest instanceof File)
            dest = [...dest.path, dest.name];
        if (dest instanceof Dir)
            dest = dest.path;
        if (dest instanceof Array)
            dest = (0, path_1.join)(...dest);
    ...
    static tmpdir() { return new Dir((0, os_1.tmpdir)()); }
    /**Returns the amount of files in a directory*/
    getSize() { return (0, fs_1.readdirSync)(this.sysPath()).length; }
    /**Checks if a directory is a symbolic link */
    isLink() { return (0, fs_1.lstatSync)(this.sysPath()).isSymbolicLink(); }
    /**Does the same as the {@link sysPath} function */
    toString() { return this.sysPath(); }
    /**Gets a new directory relative to this directory */
    getDir(..._file) { return new Dir(...this.path, ..._file); }
    /**Gets a new file relative to this directory */
    getFile(..._file) { return new File(...this.path, ..._file); }
    /**Removes this file or directory */
    rm() {
        (0, fs_1.rmSync)(this.sysPath(), { recursive: true, force: true });
        return this;
    }
    /**Copies this file or directory to a set location*/
    copyTo(dir) { (0, fs_1.copyFileSync)(this.sysPath(), dir.sysPath()); }
    /**Moves this file or directory to a set location*/
    moveTo(file) { (0, fs_1.renameSync)(this.sysPath(), file.sysPath()); return file; }
    /**Checks if this file exists */
    exists() { return (0, fs_1.existsSync)(this.sysPath()); }
    ...
    ls() {
        const res = [];
        if (this.exists()) {
            (0, fs_1.readdirSync)(this.sysPath()).forEach(e => {
                const stat = (0, fs_1.statSync)((0, path_1.join)(this.sysPath(), e));
...
class File extends Dir {
    ...
    dir() { return new Dir(...this.path); }
    /**Reads a file and returns the raw array buffer */
    readRaw() { return (0, fs_1.readFileSync)(this.sysPath()); }
    /**Reads a file and returns it as a string */
    read() { return this.readRaw().toString(); }
    /**Returns a base64 representation of the file */
    toBase64() { return (0, fs_1.readFileSync)(this.sysPath(), 'base64url'); }
    /**@override Gets the name of this file*/
    getName() { return this.name; }
    /**@override Return the system path of a file*/
    sysPath() { return (0, path_1.join)(super.sysPath(), this.name); }
    /**@override */
    javaPath() { return [...this.path, this.name].join("/"); }
    /**@override gets the hash of this file*/
    getHash(algorithm = "sha1") { return (0, crypto_1.createHash)(algorithm).update((0, fs_1.readFileSync)(this.sysPath())).digest("hex"); }
    /**@override gets the size of this file*/
    getSize() { return (0, fs_1.statSync)(this.sysPath()).size; }
    /**Reads a set file and returns a json representation of that object*/
    toJSON() {
        if (this.exists())
            return JSON.parse((0, fs_1.readFileSync)(this.sysPath()).toString());
        console.trace();
     ...
     */
    async download(url, chk, opt) {
        if (this.chkSelf(chk)) {
            await new Promise((resolve, reject) => {
                const file = (0, fs_1.createWriteStream)(this.sysPath());
                (0, node_fetch_1.default)(url, { signal: opt?.signal }).then(res => {
                    if (!res.ok)
                        reject(res.status);
    ...
    chmod() { if ((0, os_1.type)() != "Windows_NT" && this.exists())
        (0, child_process_1.execSync)(`chmod +x "${this.sysPath()}"`); }
    /**Writes data to file. Automatically converts JSON objects to parsable strings before saving them*/
    write(data) {
        if (typeof data == "object")
            data = stringify(data);
        (0, fs_1.writeFileSync)(this.sysPath(), data);
    }
    /**Loads a json object from the file system and adds some shortcut functions to make it easier to save. */
    load(def, serializer = (raw) => raw) {
        const obj = this.exists() ? serializer(this.toJSON()) : serializer(def);
    ...
    toDownloadable(url, key, chk, opt) {
        this.mkdir();
        const d = { key: key || [...this.path, this.name].join("/"), name: this.name, path: this.path, url: url, chk: {} };
        if (chk) {
            d.chk = chk;
        }
        if (opt) {
    ...
    static async process(json, zipDir) {
        const f = new this(...json.path, json.name);
        if (json.dynamic && f.exists()) {
            return;
        }
    ...
    unzip(path, exclude, zipDir) {
        ...
        return new Promise(e => {
            const s = (0, child_process_1.spawn)(get7zip(zipDir).sysPath(), com, { "cwd": (0, path_1.join)(this.getDir().sysPath()), "env": process.env });
            s.on("exit", e);
        });
    }
    /**Similar to {@link unzip}, but uses a whitelist approach */
    extract(path, files, zipDir) {
        const com = ['x', this.sysPath(), '-y', '-o' + path.sysPath()];
        files.forEach(e => {
            let f = String(e);
            if (f.endsWith("/"))
    ...
    isExecutable() {
        return new Promise((res) => (0, fs_1.access)(this.sysPath(), fs_1.constants.F_OK, (err) => res(err ? false : true)));
    }
}
exports.File = File;
let defDir = Dir.tmpdir().getDir("gmll");
function get7zip(dir = defDir) {
...
async function download7zip(dir, os, arch) {
...
exports.download7zip = download7zip;
